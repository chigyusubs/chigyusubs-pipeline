#!/usr/bin/env python3
"""Transcribe audio using faster-whisper large-v3, outputting Gemini-compatible raw JSON.

Uses the same VAD chunking as the Gemini and Qwen paths for apples-to-apples comparison.
Each chunk is transcribed independently (condition_on_previous_text=False).

Requires: system python3.12 with faster-whisper and ROCm-enabled CTranslate2.
"""
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as workdir:
        for idx, (start_s, end_s) in enumerate(chunk_bounds):
            ...
            wav_path = os.path.join(workdir, f"whisper_{idx}.wav")
            ...
            try:
                ...
                log(f"  {len(text)} chars, {len(lines)} segments, {elapsed:.1f}s")
...
def main():
    run = start_run("transcribe_faster_whisper_raw")

    parser = argparse.ArgumentParser(
        description="Transcribe audio with faster-whisper, outputting Gemini-compatible raw JSON."
    )
    ...
    parser.add_argument(
        "--glossary", default="",
        help="Path to glossary/initial_prompt text file.",
    )
    parser.add_argument(
    ...
    if args.glossary and Path(args.glossary).exists():
        initial_prompt = Path(args.glossary).read_text(encoding="utf-8").strip()
        log(f"Loaded glossary ({len(initial_prompt)} chars): {initial_prompt[:80]}...")
    ...
    log(f"Loading faster-whisper {args.model} ({args.compute_type}) on cuda...")
    t0 = time.monotonic()
